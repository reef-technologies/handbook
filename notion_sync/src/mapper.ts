...
export interface MapperContext {
  ...
  // Schedule a download of a Notion-hosted image/file (their presigned URLs
  // expire within an hour); returns the relative path it will be saved at.
  saveImage(url: string): string;
  saveFile(url: string): string;
...
function blockToMdast(block: Block, ctx: MapperContext): FlowContent[] {
  switch (block.type) {
    case 'paragraph':
      if (block.paragraph.rich_text.length === 0) return []; // blank line in Notion
      return [
    ...
    case 'image': {
      ...
      const url = block.image.type === 'file' ? ctx.saveImage(block.image.file.url) : block.image.external.url;
      return [{
        type: 'paragraph',
...
function mediaToMdast(media: MediaContent, ctx: MapperContext): FlowContent {
  ...
  const path = ctx.saveFile(media.file.url);
